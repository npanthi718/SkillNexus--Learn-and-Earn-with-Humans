  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="glass-card w-full max-w-lg p-6 sm:p-7">
        ...
        <form className="mt-5 space-y-4 text-sm" onSubmit={handleSubmit}>
          ...
          <div className="mt-2 grid grid-cols-2 gap-2">
            ...
            <button
              type="button"
              className="rounded-lg border border-white/15 px-3 py-2 text-xs hover:bg-white/10"
              onClick={() => { try { onClose && onClose(); } catch {} navigate("/auth#forgot"); }}
            >
              Forgot password?
            </button>
          </div>
        ...
        <button
          className="mt-4 w-full text-center text-xs text-nexus-200 hover:text-white"
          onClick={() => {
            setMode((m) => (m === "login" ? "register" : "login"));
            setError("");
          }}
        >
          {mode === "login" ? "New to SkillNexus? Create an account" : "Already a member? Login"}
        </button>
      </div>
    </div>
  );
